/*
AirDrop system made by NimA BastaniW
...
#define AirDropTime 1800 //Secounds
//1800s = 30 Minutes
#define AfterDropDestroyTimer 600 //Secounds
//600s = 10 Minutes
new AirDropTimer;
new AirDropTimerCount;
new LastAirDropUsed;
...
stock Get2DZoneName(Float:x, Float:y, zone[], len) //Credits to Cueball, Betamaster, Mabako, and Simon (for finetuning).
{
...
{
	CA_Init();
	printf("Random AirDrop System by Nima Bastaniw loaded !"); //DON'T REMOVE CREDITS
	//pak nakon BA HOVIAT :D
	printf("I'll drop an AirDrop every %d secound(s) !", AirDropTime);
	LastAirDropUsed = 1;
	AirDropTimerCount = 0;
	LastAirDropFindTimer = 0;
	AirDropTimer = SetTimer("AirDropCount", 1000, 1);
	return 1;
}
...
public OnFilterScriptExit()
{
	...
	printf("Random AirDrop System by Nima Bastaniw unloaded !"); //DON'T REMOVE CREDITS
	//pak nakon BA HOVIAT :D
	return 1;
}
...
{
	SendClientMessage(playerid, -1, "This server is using NimaBastaniw's AirDrop system !");//DON'T REMOVE CREDITS
	return 1;
}

forward AirDropCount();
public AirDropCount()
{
	if(LastAirDropUsed == 0)
	{
		if(LastAirDropFindTimer == AfterDropDestroyTimer)
		{
			LastAirDropUsed = 0;
			LastAirDropFindTimer = 0;
			AirDropTimerCount = 0;
			DestroyDynamicObject(LastAirDrop[adobjectid]);
			DestroyDynamic3DTextLabel(LastAirDrop[Text3Did]);
		}
		return 1;
	}
	for(new i = 0; i < 4; i++)
	if(AirDropTimerCount == AirDropTime)
	{
		AirDropTimerCount = 0;
		DropIt();
	}
...
stock DropIt()
{
	...
	if(raycl != WATER_OBJECT && raycl != 0 && strcmp(zone, "Los Santos", true))
	{
		...
		LastAirDrop[rayX] = rX;
		LastAirDrop[rayY] = rY;
		LastAirDrop[rayZ] = rayoZ;
		LastAirDrop[rayRX] = rx;
		LastAirDrop[rayRY] = ry;
		LastAirDrop[rayRZ] = rz;
		LastAirDropFindTimer = 0;
...
{
	if(objectid == LastAirDrop[adobjectid])
	{
...
{
	if(LastAirDropUsed == 1) return 1;
	new Float:x, Float:y, Float:z;
	GetDynamicObjectPos(LastAirDrop[adobjectid], x, y, z);
	SetPlayerPos(playerid, x, y, z+4.0);
	return 1;
}
CMD:resetairdrop(playerid, params[])
{
	LastAirDropUsed = 0;
	AirDropTimerCount = 0;
	DestroyDynamicObject(LastAirDrop[adobjectid]);
...
CMD:getdrop(playerid, params[])
{
	if(LastAirDropUsed == 1) return 1;
	new Float:x, Float:y, Float:z;
	GetDynamicObjectPos(LastAirDrop[adobjectid], x, y, z);
	if(IsPlayerInRangeOfPoint(playerid, 5.0, x, y, z+2))
	{
		LastAirDropUsed = 1;
		for(new i = 1; i < 4; i++)
		{
			new wp = randomWP[random(9)];
			GivePlayerWeapon(playerid, wp, 250+random(1000));
			new string[256];
			new wpname[40];
			GetWeaponName(wp, wpname, sizeof(wpname));
			format(string, sizeof(string), "[AirDrop] : You picked up %s.", wpname);
			SendClientMessage(playerid, 0x0066FFCC, string);
			DestroyDynamicObject(LastAirDrop[adobjectid]);
			DestroyDynamic3DTextLabel(LastAirDrop[Text3Did]);
		}
	}
